...
const resourceValidators = [
  ...
  body('resourceAvailable')
    .not().isEmpty().withMessage('resourceAvailable is required')
...
app.post('/api/resources', resourceValidators, async (req, res) => {
  ...
  // Ensure only 'create' action is processed
  if (action !== 'create') {
    return res.status(400).json({
      ok: false,
      error: 'Only action "create" is implemented',
    });
  }
  ...
  try {
    ...
    `;
    const params = [
      resourceName,   // store raw name
      resourceDescription, // store raw description
